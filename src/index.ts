...
export class ValidatorCreator {
    static ClassFromValidatorObject<T extends ValidatorObject>(validators: T, DefaultNilValue?: any):
        ...
        return class {
            constructor(data: ValidatorObjectCleaned<T>) {
                ...
                keys.forEach(key => {
                    this[key as keyof this] = this[key as keyof this] ?? data[key as keyof typeof data] ?? DefaultNilValue
                });
            }
        } as any;
    }
    static FunctionFromValidatorObject<T extends ValidatorObject>(validators: T, options: createValidatorFunctionOption = { DefaultNilValue: null, strict: true, throwErrors: true }) {
                ...
                if (!validator(value)) {
                    if (options.throwErrors) {
                        throw new ValidatorError(
                            `Tipo de dato invalido se requeria ${validator.name
                            } y se encontro ${typeof value} en`,
                            key.toString()
                        );
                    }
                    return false
