...
export function isArray<T>(validator: ValidatorFunction<T>) {
  return (val: any): val is T[] => {
    return Array.isArray(val) && val.every((data) => validator(data));
  };
}
export function isMap<T>(validator: ValidatorFunction<T>) {
  return (val: any): val is { [key: string]: T } => {
    return typeof val === "object" && Object.values(val).every((data) => validator(data));
  };
}
export function isTuple<T extends ValidatorFunction<any>[]>(...validators: T) {
  return (val: any): val is ValidatorFunctionsTypesArray<T> => {
    return (
      Array.isArray(val) &&
...
export function isNotNull<T>(validator: ValidatorFunction<T>) {
  return (val: any): val is NonNullable<T> => {
    return val !== null && val !== undefined && validator(val);
  };
}
export function or<T extends ValidatorFunction<any>[]>(...validators: T) {
  return (val: any): val is ValidatorFunctionType<T[number]> => {
    return validators.some((validator) => validator(val));
  };
}
export function optional<T extends ValidatorFunction<any>>(validator: T) {
  return (val: any): val is Optional<ValidatorFunctionType<T>> => {
    return val === null || val === undefined || validator(val);
  };
}
